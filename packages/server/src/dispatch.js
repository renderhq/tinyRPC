...
export async function resolveHTTPResponse(opts) {
    ...
    const results = await Promise.all(paths.map(async (p, index) => {
        try {
            ...
            const trpcError = err instanceof TRPCError
                ? err
                : new TRPCError({
...
export function createHTTPHandler(opts) {
    return async (req, res) => {
        ...
        if (cors) {
            ...
            res.setHeader('Access-Control-Allow-Methods', (corsOpts.methods || ['GET', 'POST', 'OPTIONS']).join(', '));
